...
LAB=02                                   # <-- the one line that changes per lab
...
say ""
# ---- build the checker engine into the workspace ----
if [ -f "$HERE/grader.c" ]; then
  cp -f "$HERE/grader.c" "$WS/grader.c"
  if command -v gcc >/dev/null 2>&1; then
    if gcc "$WS/grader.c" -o "$WS/grader"; then say "  grader: built"
    else say "  grader: BUILD FAILED — call the instructor"; fi
  fi
fi

say ""
say "=================================================================="
say "  WORK ONLY INSIDE:  $WS"
say "  Files created anywhere else are INVISIBLE to the checker"
say "  and will NOT be bundled. Your very next command must be:"
say ""
say "      cd $WS"
say ""
say "  then:  script session.txt   (start the recorder FIRST)"
say "=================================================================="
